...
void init_ctx(struct ctx *ctx)
{
	...
	ctx->defects = new CvPoint[NUM_DEFECTS + 1];
}

void filter_and_threshold(ctx *ctx)
{
	...
	cvInRangeS(ctx->temp_image3,
		   cvScalar(0, 30, 60, 0),		//(0, 55, 90, 255) or (0, 30, 60, 0)
		   cvScalar(20, 150, 255, 0),		//(28, 175, 230, 255) or (20, 150, 255, 0) 
		   ctx->thr_image);
	...
	if (ctx->hull) {
		...
		if (defects && defects->total) {
			...
			for (i = 0; i < defects->total; i++) {
				double d = (x - defect_array[i].depth_point->x) *
					(x - defect_array[i].depth_point->x) +
					(y - defect_array[i].depth_point->y) *
					(y - defect_array[i].depth_point->y);
